<?php 
	...
	if(isset($_POST['submit'])){
		...
		if(!empty($_POST['kind'])){
			...
			$answer = $_POST['kind'];
			if($answer=="user"){
				$sql = "SELECT u_id from user where u_email='$email' and u_password='$password'";
			}
			if($answer=="con"){
				$sql = "SELECT c_id from user where c_email='$email' and c_password='$password'";
			}
			if($answer=="auth"){
				$sql = "SELECT a_id from user where a_email='$email' and a_password='$password'";
			}
		...
		if($account['u_email']==$email && $account['u_password']==$password){
				echo "Access granted";
			}
		}
	}
...
<section class="container white-text">
	...
	<h5>Log-in</h5>
	<form class="white" action="login_two.php" method="POST" enctype="multipart/form-data">
